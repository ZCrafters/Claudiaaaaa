  ...
  return (
    <div className={`relative w-full h-screen flex flex-col items-center justify-center overflow-hidden transition-all ${shake ? 'shake' : ''}`}>
      ...
      <div className="relative z-10 w-full max-w-2xl px-6">
        ...
        >
          {availableWords.map((word, idx) => (
            <motion.button
              ...
              className="px-4 py-3 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-400 hover:to-yellow-500 text-slate-900 font-bold rounded-lg transition-all duration-300 cursor-pointer"
        ...
        {/* Drop Zone */}
        <div className="bg-slate-800/50 border-2 border-yellow-400/50 rounded-lg p-8 mb-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {Array.from({ length: NUM_SLOTS }).map((_, idx) => (
              <motion.div
                key={idx}
                className="aspect-square bg-slate-900/50 border-2 border-dashed border-yellow-300/50 rounded-lg flex items-center justify-center min-h-16 relative"
                animate={{
                  borderColor:
              ...
              >
                {selectedWords[idx] ? (
                  <motion.button
                    ...
                    className="px-3 py-2 bg-yellow-500 text-slate-900 font-bold rounded text-sm cursor-pointer hover:bg-yellow-400 transition-all"
